...
#'
#' Provides a thin management layer around the \code{predictorControls} Shiny
#' module. An environment created by
#' \code{initialize_predictor_controls_env()} tracks all
...
#' \enumerate{
#'   \item Call \code{initialize_predictor_controls_env()} once (e.g. in the
#'         Shiny server function) to obtain a shared environment.
#'   \item Call \code{create_predictor_controls()} for each model whose
...
#'         \code{get_predictor_controls_values()}.
#'   \item When you need to replace all controls (e.g. after a model reload),
#'         call \code{destroy_all_predictor_controls()} and then recreate as
#'         needed.
#' }
#'
#' @examples
...
#' module IDs across successive calls to
#' \code{destroy_all_predictor_controls()}.
#'
...
#'
#' Instantiates the UI and server for a \code{predictorControls} Shiny module,
#' registers the server in the environment, and returns both components.
#'
#' @param .env Environment created by
#'   \code{initialize_predictor_controls_env()}.
...
#' returns its reactive values, optionally wrapped in \code{shiny::isolate()} to
...
#'   parameter passed to \code{get_model_predictor_controls_id}.
#' @param rv_isolate Logical; if \code{TRUE} (the default) the reactive values
#'   are returned inside \code{shiny::isolate()}, preventing the caller from
...
get_predictor_controls_values <- function(
  ...
  if (rv_isolate) {
    shiny::isolate(predictor_ctrl$rv_values())
  } else {
