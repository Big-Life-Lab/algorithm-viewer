...
#' \code{initialize_cached_data()} and passed explicitly to the other functions.
#'
...
#' to calculate the data, while \code{data} is the actual data stored in
...
#' \code{get_cached_data()}, \code{set_cached_data()}).
#'
#' @return A cache that can be passed to other cached data functions.
#'
#' @seealso \code{get_cached_data()}, \code{set_cached_data()},
#'   \code{is_reusable_cached_data()}, \code{clear_cached_data()}
#'
#' @noRd
#' @keywords internal
...
#' exists.
#'
#' @param cache Cache created by \code{initialize_cached_data()}.
#' @param key A value (often a list of values) that identifies the specific
#'   cache entry within the cache. Hashed internally via \code{.hash_key()}.
#'
...
#'   the data should be recalculated and \code{set_cached_data} should be
#'   recalled.
...
#' Call \code{get_cached_data()} to retrieve the data.
#'
#' @param cache Cache created by \code{initialize_cached_data()}.
#' @param key A value (often a list of values) that identifies the specific
#'   cache entry within the cache. Hashed internally via \code{.hash_key()}.
