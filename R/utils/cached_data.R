...
#'
#' Provides functions for caching and reusing data. Each cache is an
#' environment created by [initialize_cached_data_env()] and
#' passed explicitly to the other functions.
#'
...
#' cache <- initialize_cached_data_env()
#'
#' params <- list(
#'   predictor             = "clc_age",
...
#' )
#' 
#' # 2. Create the key that is used to identify the entry.
#' cache_key <- list(
#'    "or",
#'    "model_1",
#'    params$predictor,
...
#' }
#'
#' # 3. Invalidate the entire cache. Should be performed whenever the cache
...
#' Initialize the cached data environment
#'
#' Creates a new environment used to store cached data across calls.
#' Pass the returned environment to other functions in this module (e.g.,
#' [get_cached_data()], [set_cached_data()]).
#'
#' @return An environment with a single element \code{cached_data},
#'   initialized to an empty list.
#'
#' @seealso [get_cached_data()], [set_cached_data()],
#'   [is_reusable_cached_data()], [clear_cached_data()]
initialize_cached_data_env <- function() {
  rlang::env(
    cached_data = list()
  )
}
...
#' exists.
#'
#' @param .env Environment created by [initialize_cached_data_env()].
...
get_cached_data <- function(.env, key) {
  .get_cached_data_entry(.env, key)$data
}
...
#'
#' @keywords internal
.get_cached_data_entry <- function(.env, key) {
  key <- .hash_key(key)
  .env$cached_data[[key]]
}
...
set_cached_data <- function(
  ...
  .env$cached_data[[.hash_key(key)]] <- entry
}
...
is_reusable_cached_data <- function(
  ...
  old_params <- .get_cached_data_entry(.env, key)$params
...
#'
#' Removes all cached entries from the environment. Should be called
#' whenever the cached data are no longer valid.
#'
#' @param .env Environment created by [initialize_cached_data_env()].
clear_cached_data <- function(.env) {
  .env$cached_data <- list()
}
