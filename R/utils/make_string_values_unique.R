...
#' @param template A [glue::glue()] template string used to rename duplicates.
#'   The template has access to `value` (the original string) and `num`
#'   (1-based counter, incremented until the result is unique among preceding
#'   values). The template can also contain addition and subtraction in
#'   the string interpolated values. Defaults to `"{value} ({num+1})"`.
#'
