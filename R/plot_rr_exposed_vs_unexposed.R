...
#'
#' @name plot_rr_exposed_vs_unexposed
#' @noRd
#' @keywords internal
NULL
  ...
  shiny::moduleServer(id, function(input, output, session) {
    ...
    output$group_controls <- shiny::renderUI({
      ...
      if (!is.null(model_definitions())) {
        ...
        # predictor values
        model_data <- utils::head(model_definitions()$models, 1)
        ...
        return(shiny::tagList(
          exposed_predictor_ctrl$ui,
          shiny::hr(),
          unexposed_predictor_ctrl$ui
        ))
      }
    })
      ...
      tryCatch(
        {
          ...
          for (model_data in selected_models()) {
            # Use the first model's reference group data for the default
            # predictor values
            exposed_model_data <- utils::head(model_definitions()$models, 1)
            exposed_model_data <-
              exposed_model_data[[names(exposed_model_data)[1]]]
    ...
    # Make sure output$group_controls is rendered before output$plot, to ensure
    # that the Exposed vs Unexposed group controls are created before plotting
    shiny::outputOptions(output, "group_controls", priority = 1)
    shiny::outputOptions(output, "plot", priority = 0)
  })
}
...
.calculate_rr_exposed_vs_unexposed_curve <- function(
  ...
  for (idx in seq_along(names(exposed_group))) {
    ...
    if (is_variable_categorical(model_data, predictor)) {
      ...
      # All continuous variables will have the same exposed values, and
      # therefore the same relative risk. To avoid the redundant information,
      # the continuous variables are not added to the curve and instead are
      # combined into the first "Overall" row that was added above.
      # cur_group <- exposed_group
      ...
      # row_comparison <- as.character(glue::glue(
      #   "{predictor_label} ({exposed_value} vs {unexposed_value})"
      # ))
      # row_comparisons[[length(row_comparisons) + 1]] <-
      #   "Exposed vs Unexposed" # row_comparison
    }
  }
    ...
    shiny::fluidRow(
      ...
      shiny::column(
        ...
        shiny::div(
          ...
          shiny::uiOutput(shiny::NS(id, "group_controls"))
        )
      )
    )
  )
