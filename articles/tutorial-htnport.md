...
## The idea behind every plot: the reference patient

Every plot answers the same underlying question:
...
Try this now: set **Predictor** to **Age** and watch any of the plots
redraw as a curve over age.
...
optional and affects the **Odds Ratio** plot only.

When you choose an interaction predictor, the plot shows how a
**one-unit increase** in that second variable *modifies* the odds ratio
of the primary predictor at each x-axis value. In other words, it
visualizes whether two predictors amplify or dampen each other’s effect,
...
- The y-axis is a real probability. If the curve reads 8% at age 50, the
  ...
  is 50.
- There is no “1.0 line” here — you are reading the risk directly, not a
  ratio.
